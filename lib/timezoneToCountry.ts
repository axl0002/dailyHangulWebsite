// IANA timezone → country name mapping, derived from the tz database (zone1970.tab).
// Covers all canonical IANA zones plus common aliases used by Intl.DateTimeFormat.
// Used by the Timezone-by-Day chart to bucket users by country.
...
export type TimezoneCountryResult =
    | { kind: 'mapped'; country: string }
    | { kind: 'utc' }
    | { kind: 'unmapped'; raw: string };

export function resolveTimezoneCountry(tz: string | null | undefined): TimezoneCountryResult | null {
    if (!tz) return null;
    const trimmed = tz.trim();
    if (!trimmed) return null;

    if (trimmed in TIMEZONE_TO_COUNTRY) {
        return { kind: 'mapped', country: TIMEZONE_TO_COUNTRY[trimmed] };
    }
...
export function timezoneToCountry(tz: string | null | undefined): string | null {
    const result = resolveTimezoneCountry(tz);
    if (!result) return null;
    if (result.kind === 'mapped') return result.country;
    if (result.kind === 'utc') return 'UTC/Other';
    return 'Unknown';
}
