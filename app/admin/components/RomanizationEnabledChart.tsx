...
// profiles.show_romanization: whether the user has romanization annotations turned on.
...
export default function RomanizationEnabledChart({ filter, dateRange = 'all' }: { filter?: ProFilter; dateRange?: DateRange }) {
    const { profiles, loading } = useProfilesCache();

    const data: ChartRow[] = useMemo(() => {
        const rows = filterProfiles(profiles, 'true', dateRange); // force Pro
        const counts: Record<string, number> = { 'Romanization on': 0, 'Romanization off': 0 };
        for (const p of rows) {
            const key = p.show_romanization ? 'Romanization on' : 'Romanization off';
            counts[key] += 1;
        }
        return (['Romanization on', 'Romanization off'] as const).map(name => ({ name, pro: counts[name] }));
    }, [profiles, dateRange]);
    ...
    if (filter === 'false') return (
        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-100 md:col-span-2 lg:col-span-1 flex flex-col items-center justify-center h-[300px]">
            <p className="text-gray-500 font-medium">Romanization Enabled</p>
    ...
    if (data.every(d => d.pro === 0)) return (
        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-100 flex flex-col items-center justify-center h-[300px]">
            <p className="text-gray-500 font-medium">No Romanization Enabled data available</p>
        </div>
    );
    ...
    return (
        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-100 md:col-span-2 lg:col-span-1">
            <div className="mb-6">
                <h3 className="text-lg font-bold text-gray-900">Romanization Enabled</h3>
