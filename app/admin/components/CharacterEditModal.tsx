...
// both stores here.
// NOTE: the table has NO id column — primary key is (sentence_id, formality).
...
export default function CharacterEditModal({ character, onClose, onSave }: CharacterEditModalProps) {
    ...
    // 's<id>' for base sentences, 'v<id>' for variants (separate id spaces).
    ...
    useEffect(() => {
        const fetchSentences = async () => {
            ...
            try {
                ...
                if (error) throw error;
                const loadedVariants: SentenceVariant[] = [];
                const loaded: ExampleSentence[] = (data || []).map(s => {
                    for (const v of (s.example_sentence_variants ?? []) as SentenceVariant[]) {
                        loadedVariants.push({ ...v });
                    }
                    const { example_sentence_variants: _drop, ...base } = s;
                    return { ...base, romanization: s.romanization ?? "" };
                });
                loadedVariants.sort((a, b) => vkey(a).localeCompare(vkey(b)));
                setSentences(loaded);
                ...
                for (const v of loadedVariants) vOriginals.set(vkey(v), { ...v });
    ...
    const handleVariantChange = (key: string, field: "korean" | "romanization", value: string) => {
        setVariants(prev => prev.map(v => (vkey(v) === key ? { ...v, [field]: value } : v)));
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            for (const v of variants) {
                ...
                const original = originalVariants.get(vkey(v));
                if (original
                ...
                if (!vData || vData.length === 0) {
                    throw new Error(`Variant update for ${vkey(v)} affected 0 rows — likely blocked by RLS policy on example_sentence_variants.`);
                }
            }
                                        ...
                                        <div className="space-y-3">
                                            <div>
                                                <label className="block text-xs font-medium text-gray-500 mb-1">
                                                    Korean
                                                    {sentence.formality && (
                                                        <span className="ml-2 text-[10px] font-bold uppercase px-1.5 py-0.5 rounded bg-gray-100 border text-gray-500">
                                                            {sentence.formality}
                                                        </span>
                                                    )}
                                                </label>
                                            ...
                                            {sentence.id !== undefined &&
                                                variants.some(v => v.sentence_id === sentence.id) && (
